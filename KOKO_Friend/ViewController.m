...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleCollapsibleViewToggle:)
                                                 name:CollapsibleViewDidToggleExpandNotification
                                               object:nil];
    ...
    // collapsibleView
    self.collapsibleView = [[UIView alloc] initWithFrame:CGRectMake(0, 88, self.view.bounds.size.width, 0)];
...
// 按鈕動作
- (void)leftButton1Tapped {}
- (void)leftButton2Tapped {}
- (void)rightButtonTapped {}
- (void)segmentChanged:(UISegmentedControl *)sender {}

#pragma mark - NSNotification
- (void)handleCollapsibleViewToggle:(NSNotification *)notification {
    BOOL isExpanded = [notification.userInfo[CollapsibleViewIsExpandedKey] boolValue];
    CGRect rect = CGRectMake(0, 88, self.view.bounds.size.width, 0);
    if (isExpanded) {
        rect.size.height = 120 + 80; // test items count
        self.collapsibleView.frame = rect;
    } else {
...
- (void)fetchData {
    ...
    if (testFlag == 0) {
        ...
        CGRect rect = self.collapsibleView.frame;
        rect.size.height = 0;
        self.collapsibleView.frame = rect;
    } else if (testFlag == 1) {
        // 好友列表1+好友列表2
    ...
    [self fetchDataFromURL:urlStr completion:^(NSArray *fetchedData) {
        if (fetchedData) {
            ...
            if (testFlag == 1) {
                ...
                [self endFetchingWithDelaySince:startTime];
                [self.collapsibleView addCollapsibleViewWithItems:self.waitArray];
            }
        } else {
            [self endFetchingWithDelaySince:startTime];
        }
...
- (void)parseFetchedData:(NSArray *)fetchedData {
    for (NSDictionary *friendDict in fetchedData) {
        FriendModel *friend = [[FriendModel alloc] initWithDictionary:friendDict];

        // 根據 status 和 fid 進行處理
        if (friend.status == 0) {
            // 加入 waitArray
            BOOL exists = NO;
            for (FriendModel *existingFriend in self.waitArray) {
                if ([existingFriend.fid isEqualToString:friend.fid]) {
                    exists = YES;

                    // 如果存在，檢查更新日期，保留較新的
                    NSDate *existingUpdateDate = existingFriend.updateDate;
                    if ([friend.updateDate compare:existingUpdateDate] == NSOrderedDescending) {
                        // 替換較舊的資料
                        [self.waitArray removeObject:existingFriend];
            ...
            // 檢查是否已存在於 dataArray
            BOOL exists = NO;
            for (FriendModel *existingFriend in self.dataArray) {
                if ([existingFriend.fid isEqualToString:friend.fid]) {
                    exists = YES;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (searchText.length == 0) {
        ...
        // 搜尋字串的姓名
        for (FriendModel *friend in self.dataArray) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FriendModel *dict = self.dataArray[indexPath.row];
    if (self.filteredArray.count > 0) {
    ...
    [cell configureWithFavoriteVisible:(dict.isTop == 1)
                          profileImage:image
                                  name:name
                  actionButton1Visible:YES
                  actionButton2Visible:(dict.status == 2)
                     moreButtonVisible:(dict.status == 1)];
